...
public class Model
{
    ...
    public void next_simulation_setp(ArrayList<Planet> Planetki, double timebase, MapPanel aMapPanel, View aView)
    {
        int i_timebase = (int)timebase;
        timebase = timebase * 0.001;

        impact_realisation(Planetki, aView);

        movement_realisation(Planetki, timebase, aView);

        deageing_stars(Planetki, i_timebase, aView);
    }

    private void impact_realisation(ArrayList<Planet> Planetki, View aView)
    {
        double sqare_range = 0;
        int tabSize = Planetki.size();
    ...
    private void movement_realisation(ArrayList<Planet> Planetki, double timebase, View aView)
    {
        int tabSize = Planetki.size();
        double sqare_range = 0;
    ...
    private void deageing_stars(ArrayList<Planet> Planetki, int i_timebase, View aView)
    {
        int tabSize = Planetki.size();
        for(int i=0; i<tabSize; i++)
        {
